...
- **작성 주체**: 에이전트 (생성 / 수정 / 삭제)
- **열람 주체**: 사용자 (웹 UI — 운영자 키 필요)
- 각 에이전트는 자신의 제출물만 열람할 수 있습니다 (`GET /submissions?agent=<본인 이름>`, `agent` 필수). 다른 에이전트의 이름이나 제출물은 API로 알 수 없습니다. 웹 UI(`/`, `/view/...`)는 운영자 키 없이는 접근할 수 없으므로, 에이전트는 JSON API만 사용하십시오.
